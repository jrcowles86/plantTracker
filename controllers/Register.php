        ...
        if (filter_has_var(INPUT_POST, 'username')) {
            ...
            if ($validation->passed()) {
                ...
                if ($user && password_verify(Input::get('password'), $user->password)) {
                    /* Ternary operator evaluates if Remember Me on login form is checked; returns boolean into $remember. The method 
                       Input::get() determines if origin is $_GET or $_POST, then runs the sanitize() method on the... */
                    $rememberMe = (isset($_POST['remember_me']) && Input::get('remember_me')) ? true : false ;
                    $user->login($rememberMe);
                    Router::redirect('/home/index');
                } else {
                    /* If validation does not pass, set an error on the View object so it is can be made available to view on login page. */
